...
for typ, values in data.items():
    ...
    # ----------------------------
    # Build stats text box
    # ----------------------------
    stats_text = f"{typ}\navg σ = {avg_stdev:.2f}\n"

    for i, (n, t, s) in enumerate(top3, start=1):
        stats_text += f"#{i}: σ={s:.2f} (N={n})\n"

    # ----------------------------
    # Place box in TOP-LEFT (no overlap)
    # ----------------------------
    plt.text(
        0.02, 0.95 if typ == "QuickSort - Random" else
        0.80 if typ == "QuickSort MoT - Random" else
        0.65,
        stats_text,
